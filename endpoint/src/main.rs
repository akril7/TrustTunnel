...
/// Reload the `credentials_file` referenced by the settings file
/// and update the running core's client registry.
fn reload_clients(settings_path: &str, core: &Core) -> io::Result<()> {
    let contents = std::fs::read_to_string(settings_path).map_err(|e| {
        io::Error::new(
            e.kind(),
            format!("Couldn't read the settings file: {}", e),
        )
    })?;

    let credentials_path =
        extract_credentials_file_path(&contents, settings_path).ok_or_else(|| {
            io::Error::other("credentials_file is not configured in the settings file")
        })?;

    let credentials_path = credentials_path
        .to_str()
        .ok_or_else(|| io::Error::other("credentials_file path is not valid UTF-8"))?;

    let clients = settings::load_clients(credentials_path)
        .map_err(|e| io::Error::other(format!("Failed to load clients: {}", e)))?;

    core.reload_clients(clients)
}

fn extract_credentials_file_path(
    settings_contents: &str,
    settings_path: &str,
) -> Option<PathBuf> {
    let value = settings_contents.parse::<toml::Value>().ok()?;
    let credentials_file = value.get("credentials_file")?.as_str()?;
